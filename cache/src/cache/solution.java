...
public class solution {
    ...
    public static class SetAssociativeCache<TKey, TValue> {
		 ...
		 * @param a2
		 */
		public void setAlgo(IReplacementAlgo a2) {
    ...
    static class CacheSet<TKey, TValue> {
        ...
        /** Gets the value associated with `key`. Throws if key not found. */
        public TValue get(TKey key, IReplacementAlgo algo) {
            // If the key is present, update the usage tracker
        	System.out.println(this.UsageTracker.toString());
            if (this.containsKey(key)) {
                algo.recordUsage(this.UsageTracker, key);
            } else {
            	return (TValue) (String.format("The key '%s' was not found", key));
//                throw new RuntimeException(String.format("The key '%s' was not found", key));
        ...
        public void set(TKey key, TValue value, IReplacementAlgo algo) {
            ...
            if (indexOfKey >= 0) {
              ...
              UsageTracker = algo.recordUsage(this.UsageTracker, key);
...
//        }
    ...
    interface IReplacementAlgo {
		 ...
		 * @param key
		 */
		public <TKey> LinkedList<TKey> recordUsage(LinkedList<TKey> usageTracker, TKey key);
        
    }

    public static class LRUReplacementAlgo implements IReplacementAlgo {
    ...
    public static class MRUReplacementAlgo implements IReplacementAlgo {
    ...
    public static class SetAssociativeCacheFactory {
        ...
        public static SetAssociativeCache<String, String> CreateStringCache(int setCount, int setSize, String replacementAlgoName) {
        	ReplacementAlgoFactory ra = new ReplacementAlgoFactory();
        	IReplacementAlgo a = ra.createReplacementAlgo(replacementAlgoName);
            SetAssociativeCache<String, String> mycache = new SetAssociativeCache<>(setCount, setSize);
            mycache.setAlgo(a);
            return mycache;
        }
   ...
   static class ReplacementAlgoFactory {
